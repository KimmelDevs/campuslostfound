...
function RejectedClaimsReturnsPage() {
  ...
  const filteredClaims = rejectedClaims.filter(claim => 
  ...
  const filteredReturns = rejectedReturns.filter(returnItem => 
    ...
    <div className="min-h-screen bg-[#f8fafc]">
      <div className="max-w-7xl mx-auto px-8 pb-12">
        <div className="flex justify-between items-center mb-8 pt-6">
          <h1 className="text-3xl font-bold text-[#2c3e50]">Rejected Claims & Returns</h1>
        ...
        <div className="flex border-b mb-6">
          ...
          >
            Rejected Returns ({rejectedReturns.length})
          </button>
        </div>
              ...
              {filteredClaims.length > 0 ? (
                <div className="space-y-4">
                  {filteredClaims.map((claim) => (
                    <div key={claim.id} className="border-l-4 border-red-500 pl-4 py-3">
                      ...
                      {claim.additionalInfo && (
                        <div className="mt-2">
                          <p className="text-xs text-gray-400">Reason for rejection</p>
                ...
                <div className="text-center py-12 text-gray-500">
                  {searchTerm ? 'No matching rejected claims found' : 'There are currently no rejected claims.'}
                </div>
              )}
            </>
          ) : (
              ...
              {filteredReturns.length > 0 ? (
                <div className="space-y-4">
                  {filteredReturns.map((returnItem) => (
                    <div key={returnItem.id} className="border-l-4 border-red-500 pl-4 py-3">
                      ...
                      {returnItem.additionalInfo && (
                        <div className="mt-2">
                          <p className="text-xs text-gray-400">Reason for rejection</p>
                          <p className="text-sm text-gray-600">{returnItem.additionalInfo}</p>
                        </div>
                      )}
                    </div>
                  ))}
                ...
                <div className="text-center py-12 text-gray-500">
                  {searchTerm ? 'No matching rejected returns found' : 'There are currently no rejected returns.'}
                </div>
              )}
            </>
          )}
...
export default RejectedClaimsReturnsPage